    ...
    def split_mail_pass(self):
        for mails in self.mail_list:
            ...
            try:
                ...
                self.save_to_file('email_list.txt',
                                mail, dirname='ResultEmail')
        ...
        for url in self.url:
            ...
            try:
                ...
                if resp.status_code == 200:
                    email_list = self.scrape(resp.text)
                    print(text.format(str(len(email_list))+' \33[0memails found'))
                    for email in email_list:
                        self.save_to_file('email_list.txt',
                                        email, dirname='ResultEmail')
                else:
                    pass
            except Exception as e:
                if e.__module__ == 'requests.exceptions':
                    print(text.format('Dead sites'))
            except:
                pass
        print('\n        \33[33;1mDone scraping and check your Result folder :) ')
...
\t\t     \33[36;1mFixed by risec666\33[0m
 ...
 [3] Scrape email from website list\n''')
    ...
    try:
        choose = input('     Choose :\33[32;1m ')
        if re.search(r'[1,2,3]', choose):
            list_ = open(input('\n             \33[0mEnter your list file\n     \33[0m\n     File :\33[32;1m '), 'r').read().splitlines()
            chosedixct = {
                '1': SplitMail(*list_).split_mail_pass,
                '2': SplitMail(*list_).filter_domain,
                '3': ScraperEmail(*list_).make_request
            }
            chosedixct[choose]()
        else:
            print('Wrong choose')
